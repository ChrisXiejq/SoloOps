...
- SQLAlchemy Repository：持久化存储使用 MySQL/Baota/RDS MySQL；单元测试可用内存仓库。
...
使用宝塔 MySQL 时，在宝塔中新建 `soloops` 数据库和 `soloops` 用户，字符集选择 `utf8mb4`，然后配置：
...
真实部署时，应用运行在 ECS 或宝塔 Python 项目；数据使用 MySQL、Tair/Redis 和 OSS；使用 RAM Role/STS 临时凭证。具体步骤见 [docs/architecture.md](docs/architecture.md) 与 [docs/iam.md](docs/iam.md)。
