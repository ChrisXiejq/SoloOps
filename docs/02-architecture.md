...
- 支持从本地 Mock 演示平滑演进到真实云账号只读巡检。
- 将云 Provider、规则引擎、Planner、审批和执行器解耦。
...
flowchart TB
  ...
  SCANNER --> RULES["Rule Engine"]
  SCANNER --> PROVIDER["CloudProvider Port"]
  PROVIDER --> MOCK["Mock Provider"]
  PROVIDER -. read only .-> ALI["Alibaba Cloud APIs"]
...
| Scanner | 拉取资源快照、指标、配置和日志元数据 | 生成写操作 |
| Rule Engine | 确定性规则、阈值、去重、严重级别 | 调用 LLM 自由判断 |
| Planner | 根据 Playbook 生成修复计划 | 绕过白名单 |
| LLM Gateway | 风险解释、摘要、面向用户的说明 | 决定是否执行 |
| Approval Gate | 审批校验、职责分离、审批记录 | 修改执行参数 |
| Executor | 执行受控 Playbook、幂等、回滚入口 | 任意 Shell/SSH |
...
sequenceDiagram
  ...
  API->>Q: enqueue scan_job
  API-->>U: scan_id
  W->>P: list resources / metrics
  P-->>W: snapshots
  W->>R: evaluate snapshots
...
sequenceDiagram
  ...
  EX->>C: assume short-lived write role
  EX->>C: mutate exact resource
...
### ADR-002：执行器只接受 Playbook

Executor 不接受自然语言命令，不接受任意 Shell，不接受模型拼接的 API 参数。所有动作由 Playbook Registry 定义输入 Schema 和权限边界。

### ADR-003：读写 Provider 分离
